/**
 * HorseManager class : manages the horse and it's animations
 *
...
import {ANIMATION, START, TYPE} from "../utils/defined/Constants";
...
class HorseManager {
    ...
    constructor(scene: THREE.Scene, horseGLTF: GLTF) {
        ...
        this.setupHorseScene(horse);

        // Maya/Blender has a coordinate system where Y is Up, ThreeJS Z is Up
        // Therefore, we place the gltf object into a group and use the group for tweening
        ...
        this.setupActions(horseGLTF);
        this.activateAllActions();
    }

    /**
     * Set up the horse scene's position, scale, rotation, and axis.
     * @param horse
     */
    private setupHorseScene(horse: THREE.Group) {
    ...
    private setupActions(horseGLTF: GLTF) {
        this.idleAction = this.mixer.clipAction(horseGLTF.animations[ANIMATION[""]]);
        this.walkAction = this.mixer.clipAction(horseGLTF.animations[ANIMATION['Walk']]);
        this.trotAction = this.mixer.clipAction(horseGLTF.animations[ANIMATION['Trot']]);
        this.rightLeadAction = this.mixer.clipAction(horseGLTF.animations[ANIMATION['Canter Right Lead']]);
        this.leftLeadAction = this.mixer.clipAction(horseGLTF.animations[ANIMATION['Canter Left Lead']]);
    ...
    /**
     * Resets the horse's position and inits the halt animation
     */
    resetHorsePosition() {
        this.activateAllActions();
